...
const STATUS_CONFIG = {
    PENDIENTE:      { label: "Pendiente",      badge: "bg-amber-100 text-amber-800",     sel: "bg-amber-100 text-amber-800 border-amber-400" },
    CONFIRMADO:     { label: "Confirmado",     badge: "bg-blue-100 text-blue-800",       sel: "bg-blue-100 text-blue-800 border-blue-400" },
    EN_PREPARACION: { label: "En preparación", badge: "bg-violet-100 text-violet-800",   sel: "bg-violet-100 text-violet-800 border-violet-400" },
    LISTO:          { label: "Listo",          badge: "bg-emerald-100 text-emerald-800", sel: "bg-emerald-100 text-emerald-800 border-emerald-400" },
    EN_CAMINO:      { label: "En camino",      badge: "bg-cyan-100 text-cyan-800",       sel: "bg-cyan-100 text-cyan-800 border-cyan-400" },
    ENTREGADO:      { label: "Entregado",      badge: "bg-green-100 text-green-800",     sel: "bg-green-100 text-green-800 border-green-400" },
    CANCELADO:      { label: "Cancelado",      badge: "bg-red-100 text-red-800",         sel: "bg-red-100 text-red-800 border-red-400" },
};
...
// mode: 'create' | 'view' | 'edit' | 'status'
export default function OrderModal({ isOpen, onClose, order = null, mode = "view" }) {
    ...
    const inputCls = "w-full px-3.5 py-2.5 rounded-xl border-[1.5px] border-gray-200 bg-gray-50 text-sm text-gray-900 outline-none focus:border-orange-400 transition-colors placeholder:text-gray-400";

    // ── Handlers ───────────────────────────────────────────────────────────────
    const handleSave = async () => {
        setError(null);
        const payload = { ...form, details: form.details.filter(d => d.dish) };
        const res = isCreate ? await createOrder(payload) : await updateOrder(order._id, payload);
    ...
    const showStatusActions =
        (isStatus || (!isCreate && !isStatus && activeTab === "status")) && !isView && !confirmCancel;
        ...
        >
            <div className="bg-white rounded-2xl w-full max-w-xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto px-7 py-5 space-y-4">
                    ...
                    {/* TAB: Información */}
                    {(activeTab === "info" || isCreate) && !isStatus && (isCreate || activeTab === "info") && (
                        <>
                            {/* Tipo de orden */}
                            <div className="space-y-2">
                                <label className="text-[10px] font-bold tracking-widest text-gray-400">TIPO DE ORDEN</label>
                            ...
                            {form.orderType === "EN_MESA" && (
                                <div className="space-y-1.5">
                                    <label className="text-[10px] font-bold tracking-widest text-gray-400">MESA (ID)</label>
                                    <input className={inputCls} value={form.table}
                                        onChange={e => setField("table", e.target.value)}
                                        disabled={isView} placeholder="ID de la mesa" />
                                </div>
                            )}
                            ...
                            {form.orderType === "DOMICILIO" && (
                                <div className="space-y-1.5">
                                    <label className="text-[10px] font-bold tracking-widest text-gray-400">DIRECCIÓN DE ENTREGA</label>
                                    <div className="grid grid-cols-2 gap-2.5">
                                        {[
                                            ...
                                            <input key={key}
                                                ...
                                                onChange={e => setField("deliveryAddress", { ...form.deliveryAddress, [key]: e.target.value })}
                                                disabled={isView} placeholder={ph} />
                                        ))}
                                        <input className={inputCls} value={form.deliveryPhone}
                                            onChange={e => setField("deliveryPhone", e.target.value)}
                                            disabled={isView} placeholder="Teléfono de entrega" />
                                    </div>
                                </div>
                            )}
                            ...
                            {/* Notas */}
                            <div className="space-y-1.5">
                                <label className="text-[10px] font-bold tracking-widest text-gray-400">NOTAS</label>
                                <textarea
                            ...
                            {canEdit && (
                                <div className="space-y-1.5">
                                    <label className="text-[10px] font-bold tracking-widest text-gray-400">CUPÓN (OPCIONAL)</label>
                                    <input className={inputCls} value={form.couponCode}
                            ...
                            {isView && order?.userInfo && (
                                <div className="bg-gray-50 rounded-xl p-3.5 border border-gray-100">
                                    <p className="text-[10px] font-bold tracking-widest text-gray-400 mb-1">CLIENTE</p>
                    ...
                    {/* TAB: Platillos */}
                    {(isCreate || activeTab === "items") && !isStatus && (isCreate || activeTab === "items") && (
                        <>
                            <div className="flex items-center justify-between">
                                <label className="text-[10px] font-bold tracking-widest text-gray-400">PLATILLOS</label>
                                {canEdit && (
                                    <button
                                        onClick={addDetail}
                            ...
                            <div className="space-y-2.5">
                                {form.details.map((d, i) => (
                                    <div key={i} className="bg-gray-50 rounded-xl p-3.5 border border-gray-100 space-y-2">
                                        <div className="grid grid-cols-[2fr_1fr_1fr] gap-2">
                                            <input className={`${inputCls} bg-white`} value={d.dish}
                                                onChange={e => setDetail(i, "dish", e.target.value)}
                                                disabled={isView} placeholder="ID del platillo" />
                                            <input type="number" min="1" className={`${inputCls} bg-white`}
                                                value={d.quantity}
                                                onChange={e => setDetail(i, "quantity", Number(e.target.value))}
                                                disabled={isView} placeholder="Cant." />
                                            <input type="number" min="0" step="0.01" className={`${inputCls} bg-white`}
                                                value={d.unitPrice}
                                                onChange={e => setDetail(i, "unitPrice", parseFloat(e.target.value))}
                                                disabled={isView} placeholder="Precio" />
                                        </div>
                                        <div className="flex items-center gap-2">
                                            <input className={`${inputCls} bg-white flex-1`}
                                                value={d.specialInstructions}
                                                onChange={e => setDetail(i, "specialInstructions", e.target.value)}
                                                disabled={isView} placeholder="Instrucciones especiales" />
                                            ...
                                            <span className="text-sm font-bold text-gray-700 min-w-[64px] text-right flex-shrink-0">
                            ...
                            <div className="flex items-center justify-between pt-3 border-t border-dashed border-gray-200">
                                ...
                                <span className="text-2xl font-extrabold text-orange-500">Q{total.toFixed(2)}</span>
                            </div>
                        </>
                    )}
                    ...
                    {(activeTab === "status" || isStatus) && !isCreate && (
                        <>
                            ...
                            {order && statusCfg && (
                                <div className="bg-gray-50 rounded-xl p-3.5 border border-gray-100">
                                    <p className="text-[10px] font-bold tracking-widest text-gray-400 mb-2">ESTADO ACTUAL</p>
                            ...
                            {!isView && (
                                <>
                                    <label className="text-[10px] font-bold tracking-widest text-gray-400 block">CAMBIAR A</label>
                            ...
                            {/* Zona de peligro */}
                            {!isView && order?.status !== "CANCELADO" && order?.status !== "ENTREGADO" && (
                                <div className="pt-4 border-t border-gray-100 space-y-3">
                                    <p className="text-[10px] font-bold tracking-widest text-red-400">ZONA DE PELIGRO</p>
                                    {!confirmCancel ? (
                                        <button
                    ...
                    {success && <p className="text-sm text-green-700 font-semibold bg-green-50 border border-green-200 rounded-xl px-4 py-2.5">{success}</p>}
                </div>
